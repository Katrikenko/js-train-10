...
function customAssign(...objects) {
  ...
  const uniteObj = Object.assign(...objects);

  for (const value in uniteObj) {
    if (typeof uniteObj[value] === "number") {
      uniteObj[value] = String(uniteObj[value]);
    }
  }
  return uniteObj;
}
...
function customObjectFromEntries(entries) {
  ...
  const process = entries.map(([key, value]) => {
  ...
  const obj = Object.fromEntries(process);
  return obj;
}
...
function checkAndPreventExtensions(obj) {
  ...
  const verifyObj = Object.isExtensible(obj);
  return verifyObj;
}
...
function sealAndCheck(obj) {
  ...
  Object.seal(obj);
  const isSealObj = Object.isSealed(obj);
  return isSealObj;
}
